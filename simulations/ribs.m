...
sensor_data.p_rms = sensor_data.p_rms(1 + pml_size:end - pml_size, 1 + pml_size:end - pml_size);
...
sensor_data.p_max(slit_mask == 1) = mx;
imagesc(y_ax, x_ax, sensor_data.p_max, [-mx, mx]);
colormap(getColorMap);
ylabel('x-position [mm]');
xlabel('y-position [mm]');
axis image;
...
sensor_data.p_rms(slit_mask == 1) = mx;
imagesc(y_ax, x_ax, sensor_data.p_rms, [-mx, mx]);
colormap(getColorMap);
ylabel('x-position [mm]');
xlabel('y-position [mm]');
axis image;
title('Root Mean Squared Pressure');